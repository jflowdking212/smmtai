  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div>
        ...
        <p className="text-neutral-500 dark:text-neutral-400 mt-1">AI-powered analysis of your content performance across all platforms</p>
      </div>
      ...
      {!hasData ? (
        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-neutral-900 dark:text-white mb-2">No Performance Data Yet</h3>
        ...
        <>
                ...
                {recommendations.map((rec) => (
                  ...
                  >
                    <div className="flex items-start gap-2 mb-2">
                      ...
                      <h4 className="text-sm font-semibold text-neutral-900 dark:text-white leading-tight">{rec.title}</h4>
                    </div>
                    <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-3 line-clamp-3">{rec.body}</p>
                    <div className="flex gap-2">
                      <button
                        onClick={() => updateRecommendation(rec.id, 'acted')}
                        className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 text-xs rounded-lg bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 border border-green-200 dark:border-green-800/30 hover:bg-green-100 dark:hover:bg-green-900/50 transition-colors"
                      >
                        <ThumbsUp className="w-3 h-3" /> Act on it
                      </button>
                      <button
                        onClick={() => updateRecommendation(rec.id, 'dismissed')}
                        className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 text-xs rounded-lg bg-neutral-100 dark:bg-neutral-800 text-neutral-500 dark:text-neutral-400 border border-neutral-200 dark:border-neutral-700 hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors"
                      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Engagement Heatmap */}
            {patterns.heatmap && (
              <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-6 lg:col-span-2">
                  ...
                  <div className="min-w-[600px]">
                    ...
                    <div className="flex mb-1 ml-10">
                      {[0, 3, 6, 9, 12, 15, 18, 21].map((h) => (
                        <div key={h} className="flex-1 text-[10px] text-neutral-400 dark:text-neutral-600 text-center">
                          {h}:00
                        </div>
                      ))}
                    </div>
                    ...
                    {patterns.heatmap.map((row, dayIdx) => (
                      <div key={dayIdx} className="flex items-center gap-1 mb-0.5">
                        <span className="text-[10px] text-neutral-400 dark:text-neutral-500 w-8 text-right shrink-0">{DAY_NAMES[dayIdx]}</span>
                    ...
                    return (
                      <div key={ct.type}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-neutral-700 dark:text-neutral-300 capitalize">{ct.type}</span>
                ...
                <div className="space-y-3">
                  {patterns.bestTopics.slice(0, 6).map((tp, i) => {
                    ...
                    return (
                      <div key={tp.topic}>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs text-neutral-700 dark:text-neutral-300 capitalize">{tp.topic}</span>
              ...
              <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-6">
                ...
                <div className="space-y-3">
                  {patterns.platformTrends.map((pt) => (
                    <div key={pt.platform} className="flex items-center justify-between p-3 bg-neutral-50 dark:bg-neutral-800/50 rounded-lg">
                      <span className="text-xs text-neutral-700 dark:text-neutral-300 capitalize font-medium">{pt.platform}</span>
                      <div className="flex items-center gap-3">
                        <span className="text-xs text-neutral-500 dark:text-neutral-400">{pt.avgEngRate.toFixed(1)}%</span>
                  ...
                  {patterns.weeklyTrend.map((wk, i) => {
                    ...
                    return (
                      <div key={wk.week} className="flex-1 flex flex-col items-center gap-1">
                        <span className="text-[9px] text-neutral-400 dark:text-neutral-500">{wk.avgEngRate.toFixed(1)}%</span>
                        <div
                          className="w-full rounded-t-sm transition-all duration-500"
                          style={{
                            height: `${Math.max(4, height)}%`,
                            background: i === patterns.weeklyTrend.length - 1 ? '#ef4444' : 'rgb(209, 213, 219)',
                          }}
                        />
                        <span className="text-[9px] text-neutral-400 dark:text-neutral-600">{wk.week.slice(5)}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="space-y-3">
                  {patterns.abTestResults.map((test, i) => (
                    <div key={i} className="bg-neutral-50 dark:bg-neutral-800/50 rounded-lg p-4">
                      <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-2 capitalize">
                        Testing: <span className="text-neutral-900 dark:text-white font-medium">{test.variableType}</span>
                      </p>
                      <div className="grid grid-cols-2 gap-3">
                        <div className={`rounded-lg p-3 border ${
                          ...
                          <p className="text-sm font-semibold text-neutral-900 dark:text-white capitalize">{test.variationA}</p>
                          <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">{test.engRateA.toFixed(1)}% engagement</p>
                        </div>
                        <div className={`rounded-lg p-3 border ${
                          test.winner === 'B'
                            ? 'border-green-300 dark:border-green-800/50 bg-green-50 dark:bg-green-900/10'
                          ...
                          <p className="text-sm font-semibold text-neutral-900 dark:text-white capitalize">{test.variationB}</p>
                          <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">{test.engRateB.toFixed(1)}% engagement</p>
                        </div>
                      </div>
                      <p className="text-[10px] text-neutral-400 dark:text-neutral-600 mt-2">
