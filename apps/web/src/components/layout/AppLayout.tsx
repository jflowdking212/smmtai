...
export function AppLayout() {
  ...
  async function handleResendVerification() {
    ...
    try {
      ...
      toast.error('Error', err instanceof Error ? err.message : 'Failed to send verification email');
    } finally {
  ...
  const isSystemAdmin = user?.email === 'judeobidozie@gmail.com';
  ...
  return (
    <>
      ...
      {user && !user.emailVerified && (
        <div className="fixed top-0 left-0 right-0 z-[100] bg-amber-50 dark:bg-amber-900/30 border-b border-amber-200 dark:border-amber-700 px-4 py-2.5 flex items-center justify-between gap-4">
          ...
          <button
            ...
            className="shrink-0 text-xs font-semibold text-amber-700 dark:text-amber-300 hover:underline disabled:opacity-50 whitespace-nowrap"
          >
      ...
      <div className={`min-h-screen bg-neutral-50 transition-all ${user && !user.emailVerified ? 'pt-10' : ''}`}>
      {/* Mobile overlay */}
      {mobileOpen && (
        <div
        ...
        <header className="sticky top-0 z-30 h-16 bg-white/80 backdrop-blur-md border-b border-neutral-200/60 flex items-center justify-between px-4 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="relative p-2 rounded-xl text-neutral-500 hover:bg-neutral-100 transition-all duration-200 hover:scale-105 active:scale-95"
            >
              {isDark ? (
